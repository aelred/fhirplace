...
export default function Row({ icon, name, flags, card, type, description, isOpen, setOpen, indent, hasChildren, isLastChild, button }: Props) {
    ...
    const backgroundImage = `url(${process.env.PUBLIC_URL}/hl7fhir/tbl_bck${thisIndent.map(b => +b).join("")}.png)`
    ...
    return <tr style={{ border: "0px #F0F0F0 solid", padding: "0px", verticalAlign: "top" }}>
        <td style={{ whiteSpace: "nowrap", backgroundImage }} className="hierarchy">
            ...
            {indent.length > 0 && (<VJoin isOpen={isOpen} isLastChild={isLastChild} setOpen={v => setOpen && setOpen(v)} />)}
            {icon}
            {" "}
            {name}
            {button}
